...
/*   Updated: 2025/11/26 14:37:12 by gabrgarc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fractol_init(t_fractol *fractol)
{
	...
	fractol->img.addr = mlx_get_data_addr(
			...
			&fractol->img.endian);
	fractol->coordinates.r = (2.0 - (-2.0)) / WIDTH;
	fractol->coordinates.i = (-2.0 - 2.0) / HEIGHT;
}
...
void	fractol_render(t_fractol *fractol)
{
	...
	while (y < HEIGHT)
	{
		...
		while (x < WIDTH)
		{
			out = handle_pixel(x, y, fractol->coordinates);
			if (out == ITERATIONS)
				*pixel_ptr = BLACK;
			else
...
int	handle_pixel(int x, int y, t_complex coordinates)
{
	...
	c.r = -2.0 + (x * coordinates.r);
	c.i = 2.0 + (y * coordinates.i);
	i = 0;
	while (i < ITERATIONS)
	{
